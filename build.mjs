...
import { ORIGIN, BASE, CONTACT } from "./src/site.mjs";
...
async function main() {
  ...
  await copyFile(new URL("./static/icon.svg", import.meta.url), out("/icon.svg"));
  await copyFile(new URL("./static/og.png", import.meta.url), out("/og.png"));
  ...
  await writeFile(out("/feed.xml"),
    `<?xml version="1.0" encoding="UTF-8"?>\n`
    + `<rss version="2.0"><channel>\n`
    + `<title>Salary Crossing analysis</title>\n`
    + `<link>${ORIGIN}${BASE}/insights/</link>\n`
    + `<description>Original, calculation-led analysis of salaries, tax, mortgages and international take-home pay.</description>\n`
    + `<language>en-gb</language>\n`
    + articles.map((a) => {
      const link = `${ORIGIN}${BASE}/insights/${a.slug}/`;
      return `<item><title>${xml(a.title)}</title><link>${link}</link><guid isPermaLink="true">${link}</guid><pubDate>${new Date(`${a.datePublished}T12:00:00Z`).toUTCString()}</pubDate><description>${xml(a.description)}</description></item>`;
    }).join("\n")
    + `\n</channel></rss>\n`);

  await writeFile(out("/llms.txt"), `# Salary Crossing

> Free salary, take-home pay and mortgage calculators using published tax rates for the UK, Dubai, Australia, New York, California and Texas.

## Calculators

- [International salary comparison](${ORIGIN}${BASE}/): Find the gross salary needed abroad to match current take-home pay, with optional user-entered living costs.
- [UK take-home pay calculator](${ORIGIN}${BASE}/take-home/): Calculate Income Tax, National Insurance, pension and student-loan deductions for the current tax year.
...
- [Original analysis](${ORIGIN}${BASE}/insights/): Calculation-led articles whose tables are generated by the same tax engine as the calculators.
- [Embed Salary Crossing](${ORIGIN}${BASE}/use-our-numbers/): Free calculator embed for publishers, with no signup or API key.
- [Adviser workspace](${ORIGIN}${BASE}/advisers/): Founding pilot for sourced, adviser-approved international compensation reports.
- [Adviser resources and sample reports](${ORIGIN}${BASE}/advisers/resources/): Practical field guides and fictional, engine-generated compensation reports for professional review.
- [Methodology and sources](${ORIGIN}${BASE}/about/): Scope, limitations and links to the tax authorities used.
- [RSS feed](${ORIGIN}${BASE}/feed.xml): New analysis in a machine-readable feed.

## Scope

Salary Crossing models standard employment income, income tax and compulsory payroll contributions. It separates retirement benefits from spendable cash. It does not decide whether somebody should move, determine tax residency, or provide personal tax, mortgage or immigration advice.

All tools are free. Questions and corrections: ${CONTACT}.
`);
  ...
  await mkdir(out("/advisers"), { recursive: true });
  await writeFile(out("/advisers/index.html"), adviserPage());
  const adviserResources = buildAdviserResources(fx);
  await mkdir(out("/advisers/resources"), { recursive: true });
  await writeFile(out("/advisers/resources/index.html"), adviserResources.resourceIndex);
  for (const article of adviserResources.articles) {
    await mkdir(out(`/advisers/resources/${article.slug}`), { recursive: true });
    await writeFile(out(`/advisers/resources/${article.slug}/index.html`), article.html);
  }
  await mkdir(out("/advisers/sample-reports"), { recursive: true });
  await writeFile(out("/advisers/sample-reports/index.html"), adviserResources.reportIndex);
  for (const report of adviserResources.reports) {
    await mkdir(out(`/advisers/sample-reports/${report.slug}`), { recursive: true });
    await writeFile(out(`/advisers/sample-reports/${report.slug}/index.html`), report.html);
    await writeFile(out(`/advisers/sample-reports/${report.slug}/report.pdf`), report.pdf);
  }
