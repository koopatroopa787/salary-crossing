...
function verdict(c) {
  ...
  const move = l.betterOff ? "keep more" : "keep less";
  const gap = Math.abs(l.difference);
  return `Move on the same salary and you would ${move}: `
    + `${fmt(c.from.gross, c.from)} converts to ${fmt(l.gross, c.to)}, which leaves `
    + `${fmt(l.net, c.to)} after ${c.to.name === "United Arab Emirates" ? "nothing" : "tax"} — `
...
<header class="masthead">
  ...
  <p class="verdict">To take home the same as ${fmt(gross, a)} does in ${a.name}, you need <b>${fmt(c.to.gross, b)}</b> in ${b.name}. ${verdict(c)}</p>
</header>
  ...
  return shell({
    ...
    jsonLd: {
      ...
      mainEntity: [{
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: `You need about ${fmt(c.to.gross, b)} in ${b.name} to take home the same as ${fmt(gross, a)} does in ${a.name}. ${fmt(gross, a)} leaves ${fmt(c.from.net, a)} after tax; ${fmt(c.to.gross, b)} leaves the equivalent ${fmt(c.to.net, b)}.`,
        },
      }],
    },
  });
